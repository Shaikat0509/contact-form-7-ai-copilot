...
final class OpenAIProvider extends AbstractProvider {
	 ...
	 *
	 * @throws ProviderException If the request fails or OpenAI returns an error.
	 */
	public function generate( string $system_prompt, string $user_prompt ): string {
		...
		if ( ! is_string( $content ) || '' === trim( $content ) ) {
			throw new ProviderException(
				esc_html__( 'OpenAI returned an empty response.', 'shaikat-ai-inbox-for-contact-form-7' )
			);
		}
	...
	public function list_models(): array {
		...
		$exclude = array( 'embedding', 'whisper', 'tts', 'dall-e', 'moderation', 'davinci', 'babbage', 'audio', 'realtime', 'transcribe' );
	...
	/**
	 * Returns the standard request headers for OpenAI.
	 *
