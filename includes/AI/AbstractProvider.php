...
abstract class AbstractProvider implements AIProviderInterface {
	...
	protected function require_api_key(): void {
		if ( '' === $this->api_key ) {
			throw new ProviderException(
				esc_html__( 'No API key has been configured for this provider.', 'olmbox-ai-inbox-for-contact-form-7' )
			);
		}
	}
	...
	protected function require_configured(): void {
		...
		if ( '' === $this->model ) {
			throw new ProviderException(
				esc_html__( 'No model has been configured for this provider.', 'olmbox-ai-inbox-for-contact-form-7' )
			);
		}
	}
		...
		if ( is_wp_error( $response ) ) {
			throw new ProviderException(
				sprintf(
					...
					esc_html__( 'Could not reach the AI provider: %s', 'olmbox-ai-inbox-for-contact-form-7' ),
	...
	protected function extract_error_message( array $body, int $code ): string {
		...
		return sprintf(
			...
			esc_html__( 'The provider returned an unexpected response (HTTP %d).', 'olmbox-ai-inbox-for-contact-form-7' ),
			$code
		);
	}
