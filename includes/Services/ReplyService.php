...
final class ReplyService {
	...
	public function send( array $entry, string $reply_text, int $user_id ): array {
		...
		if ( '' === $to || ! is_email( $to ) ) {
			return array(
				'success' => false,
				'message' => __( 'This submission has no valid visitor email address to reply to.', 'olmbox-ai-inbox-for-contact-form-7' ),
			);
		}
		...
		if ( '' === trim( $reply_text ) ) {
			return array(
				'success' => false,
				'message' => __( 'The reply cannot be empty.', 'olmbox-ai-inbox-for-contact-form-7' ),
			);
		}

		$subject = sprintf(
			/* translators: %s: site name */
			__( 'Re: Your message to %s', 'olmbox-ai-inbox-for-contact-form-7' ),
		...
		if ( ! $sent ) {
			return array(
				'success' => false,
				'message' => $mail_error ?? __( 'wp_mail() reported failure. Check your site\'s outgoing mail / SMTP configuration.', 'olmbox-ai-inbox-for-contact-form-7' ),
			);
		}
		...
		return array(
			'success' => true,
			'message' => __( 'Reply sent.', 'olmbox-ai-inbox-for-contact-form-7' ),
		);
	}
}
