...
final class AIService {
	...
	public function analyze( AIProviderInterface $provider, string $system_prompt, string $submission_text ): array {
		...
		if ( '' === $summary && '' === $reply ) {
			throw new ProviderException(
				esc_html__( 'The AI response did not contain a usable summary or suggested reply.', 'olmbox-ai-inbox-for-contact-form-7' )
			);
		}
	...
	private function parse_response( string $raw ): array {
		...
		if ( ! is_array( $decoded ) ) {
			throw new ProviderException(
				esc_html__( 'The AI response was not valid JSON.', 'olmbox-ai-inbox-for-contact-form-7' )
			);
		}
