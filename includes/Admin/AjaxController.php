...
final class AjaxController {
	...
	public function handle_test_connection(): void {
		if ( ! current_user_can( Menu::CAPABILITY ) ) {
			wp_send_json_error( array( 'message' => __( 'You do not have permission to do this.', 'olmbox-ai-inbox-for-contact-form-7' ) ), 403 );
		}

		if ( ! check_ajax_referer( self::NONCE_ACTION, 'nonce', false ) ) {
			wp_send_json_error( array( 'message' => __( 'Security check failed. Please reload the page and try again.', 'olmbox-ai-inbox-for-contact-form-7' ) ), 403 );
		}

		$provider = isset( $_POST['provider'] ) ? sanitize_key( wp_unslash( $_POST['provider'] ) ) : '';

		if ( ! array_key_exists( $provider, Repository::PROVIDERS ) ) {
			wp_send_json_error( array( 'message' => __( 'Please choose a valid provider.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
		}
	...
	public function handle_list_models(): void {
		...
		if ( empty( $models ) ) {
			wp_send_json_error( array( 'message' => __( 'No models were returned by the provider.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
		}
	...
	public function handle_send_reply(): void {
		...
		if ( null === $entry ) {
			wp_send_json_error( array( 'message' => __( 'That submission could not be found.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
			return;
		}
	...
	public function handle_save_draft(): void {
		...
		if ( $this->submissions->save_draft_reply( $id, $reply ) ) {
			wp_send_json_success( array( 'message' => __( 'Draft saved.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
		}

		wp_send_json_error( array( 'message' => __( 'Could not save the draft.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
	}
	...
	public function handle_mark_reviewed(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Marked as reviewed.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
	}
	...
	public function handle_archive(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Archived.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
	}
	...
	public function handle_delete_submission(): void {
		...
		wp_send_json_success( array( 'message' => __( 'Deleted.', 'olmbox-ai-inbox-for-contact-form-7' ) ) );
	}
